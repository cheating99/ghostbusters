    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        #find the most likely position for each ghost
        mostLikelyGhostPositions = []
        for ghostBelief in livingGhostPositionDistributions:
            mostLikelyGhostPositions.append(ghostBelief.argMax())
        ...
        #find the action that brings pacman closest to the closest ghost
        bestAction = None
        closestDistance = float("inf")
        for action in legal:
